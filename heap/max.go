...
func (mh *maxHeap) Peek() (int, error) {
	if mh.IsEmpty() {
		err := errors.New("max heap is empty")
		return 0, err
	}
...
// 父结点为(i - 1) / 2
// 1. 将元素插入到最后一个
// 2. 与父结点进行比较，若比父结点小则插入成功
//    若比父结点大，则与父结点进行交换
// 3. 直到与根结点进行比较
func (mh *maxHeap) Push(value int) error {
	if mh.IsFull() {
...
func (mh *maxHeap) Delete(index int) error {
	if mh.IsEmpty() {
...
// shiftUp 如果一个节点比它的父节点大
// 则将它同父节点交换位置，并返回true
// 如果没有进行过交换，则返回false
// 经常被循环调用，所以方法内部不加锁
// 确保调用该方法的代码加锁
func (mh *maxHeap) shiftUp(index int) bool {
	if index == 0 { // 已经到根结点了，不需要交换
		return false
	}
	parent := (index - 1) / 2
...
// 经常被循环调用，所以方法内部不加锁
// 确保调用该方法的代码加锁
func (mh *maxHeap) shiftDown(index int) (int, bool) {
	...
	// 两个孩子结点都存在，从中选一个较大者
	var bigger int
	// false 左孩子结点大， true 右孩子结点大
	var flag bool
	if mh.data[left] > mh.data[right] {
		bigger = mh.data[left]
	} else {
		bigger = mh.data[right]
		flag = true
	}
	// 与较大的孩子结点进行比较，如果比孩子结点小则进行交换
	if mh.data[index] < bigger {
		if flag {
			// 右孩子结点大，和右孩子结点进行交换
...
// BuildMaxHeap 根据给定的数组创建大顶堆
// 从数组中的第二个元素开始进行Push操作
func BuildMaxHeap(data []int) *maxHeap {
	length := len(data)
	mh := &maxHeap{
		length: length,
		size:   length,
